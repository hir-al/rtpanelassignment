...
function rtp_default_post_meta($placement = 'top')
{
	if ('post' == get_post_type() && !rtp_is_bbPress()) {
		...
		if (rtp_has_postmeta($position)) {
            ...
            <div class="clearfix post-meta post-meta-<?php echo $placement; ?>">
			<?php
			if ('bottom' == $placement) rtp_hook_begin_post_meta_bottom();
			else rtp_hook_begin_post_meta_top();
			...
			if ('bottom' == $placement) rtp_hook_end_post_meta_bottom();
			else rtp_hook_end_post_meta_top();
?>
	...
	elseif (!rtp_is_bbPress()) {
		if (get_edit_post_link() && ('top' == $placement)) { ?>
			...
			rtp_hook_end_post_meta_top(); ?></div><?php
		}
	}
}
...
function rtp_default_nav_menu()
{
	...
	echo '<h3 class="menu-toggle">Menu</h3>';
	...
	if (function_exists('wp_nav_menu') && has_nav_menu('primary')) {
		wp_nav_menu(array(
			'container' => '',
			'menu_class' => 'menu rtp-nav-container clearfix',
			'menu_id' => 'rtp-nav-menu',
			'theme_location' => 'primary',
			'depth' => apply_filters('rtp_nav_menu_depth', 4)
		));
	}
	else {
...
add_action('rtp_hook_primary_menu', 'rtp_default_nav_menu'); // Adds default nav menu after #header

function rtp_secondary_nav_menu()
{
	echo '<nav id="rtp-secondary-menu" role="navigation" class="rtp-secondary-nav-wrapper' . apply_filters('rtp_mobile_nav_support', ' rtp-mobile-nav') . '">';
	rtp_hook_begin_primary_menu();
	/* Call wp_nav_menu() for Wordpress Navigaton with fallback wp_list_pages() if menu not set in admin panel */
	if (function_exists('wp_nav_menu') && has_nav_menu('primary')) {
		wp_nav_menu(array(
			'container' => '',
			'menu_class' => 'menu rtp-secondary-nav-container clearfix',
			'menu_id' => 'rtp-nav-menu',
		...
		echo '<ul id="rtp-secondary-nav-menu" class="menu rtp-nav-container clearfix">';
		wp_list_pages(array(
			'title_li' => '',
			'sort_column' => 'menu_order',
			'number' => '5',
...
add_action('rtp_hook_header_right', 'rtp_secondary_nav_menu'); // Adds secondary nav menu after logo
...
function rtp_breadcrumb_support($text)
{
	...
	if (function_exists('bcn_display')) { // Breadcrumb NavXT Plugin Support
...
?>
        <h3 class="tagline"><?php
		bloginfo('description'); ?></h3><?php
	}
}
...
function rtp_default_single_pagination()
{
	if (!rtp_is_yarpp() && is_single() && (get_adjacent_post('', '', true) || get_adjacent_post('', '', false))) { ?>
        ...
        <?php
		if (get_adjacent_post('', '', true)) { ?><div class="left"><?php
			previous_post_link('%link', __('&larr; %title', 'rtPanel')); ?></div><?php
		} ?>
        <?php
		if (get_adjacent_post('', '', false)) { ?><div class="right"><?php
			next_post_link('%link', __('%title &rarr;', 'rtPanel')); ?></div><?php
		} ?>
	...
	if (!rtp_is_bbPress()) {
		if (!is_singular()) {
			...
			elseif (get_next_posts_link() || get_previous_posts_link()) { ?>
                ...
                <?php
				if (get_next_posts_link()) { ?><div class="left"><?php
					next_posts_link(__('&larr; Older Entries', 'rtPanel')); ?></div><?php
				} ?>
                <?php
				if (get_previous_posts_link()) { ?><div class="right"><?php
					previous_posts_link(__('Newer Entries &rarr;', 'rtPanel')); ?></div><?php
				} ?>
...
function rtp_default_comment_count()
{
	...
	if (((get_comments_number() || @comments_open()) && !is_attachment() && !rtp_is_bbPress()) || (is_attachment() && $rtp_post_comments['attachment_comments'])) { // If post meta is set to top then only display the comment count.
		 ?>
        <span class="rtp-post-comment-count"><?php
		comments_popup_link(_x('Leave a comment', 'comments number', 'rtPanel') , _x('<span>1</span> Comment', 'comments number', 'rtPanel') , _x('<span>%</span> Comments', 'comments number', 'rtPanel') , 'rtp-post-comment rtp-common-link'); ?></span><?php
	}
...
function rtp_get_sidebar_id()
{
	...
	if (function_exists('bp_current_component') && bp_current_component()) {
		...
		if ($rtp_general['buddypress_sidebar'] === "no-sidebar") {
			$sidebar_id = 0;
		}
	}
	else
	if (function_exists('is_bbpress') && is_bbpress()) {
		...
		if ($rtp_general['bbpress_sidebar'] === "no-sidebar") {
			$sidebar_id = 0;
		}
	}
...
function rtp_gallery_shortcode($output, $attr)
{
	...
	foreach($attachments as $id => $attachment) {
		...
		$output.= "<a href='" . $url[0] . "'>" . wp_get_attachment_image($id, 'thumbnail', false, array(
			"data-caption" => trim($attachment->post_excerpt)
		)) . "</a>";
		$output.= "</{$itemtag}>";
	}
...
{ ?>
	...
	$rtp_set_grid_class = apply_filters('rtp_set_full_width_grid_class', 'large-12 columns rtp-full-width-grid'); ?>
        <div class="rtp-footer-section <?php
	echo $rtp_set_grid_class; ?>">
            <p>&copy; <?php
	echo date('Y');
	echo ' - ';
	bloginfo('name'); ?>
            <em><?php
	printf(__('Designed on <a role="link" target="_blank" href="%s" class="rtp-common-link" title="rtPanel WordPress Theme Framework">rtPanel WordPress Theme Framework</a>.', 'rtPanel') , RTP_THEME_URL); ?></em></p>
        </div>
    </div><!-- #footer -->
    <?php
}
...
{ ?>
    <div class="widget sidebar-widget">
        <p>
            <?php
	_e('<strong>rtPanel</strong> is equipped with everything you need to produce a professional website. <br />It is one of the most optimized WordPress Theme Framework available today.', 'rtPanel'); ?>
        </p>
        <p class="rtp-message-success">
            <?php
	printf(__('This theme comes with free technical <a title="Click here for rtPanel Free Support" target="_blank" href="%s">Support</a> by team of 30+ full-time developers.', 'rtPanel') , 'https://rtcamp.com/support/forum/rtpanel/'); ?>
        </p>
    </div><?php
}
