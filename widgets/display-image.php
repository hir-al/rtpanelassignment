...
class DisplayImageWidget extends WP_Widget
{
	function DisplayImageWidget()
	{
		$widget_settings = array(
			'description' => 'Display Image Widget',
	...
	function widget($args, $instance)
	{
		...
		<a href="<?php
		if (!empty($linkURL)) echo $linkURL;
		else echo "#"; ?>" title="<?php
		if (!empty($imageTitle)) echo $imageTitle;
		else echo ''; ?>" target="_blank">		
			<img src="<?php
		echo $imageSrc; ?>" alt="<?php
		if (!empty($imageAlt)) echo $imageAlt;
		else echo ''; ?>" title="<?php
		if (!empty($imageTitle)) echo $imageTitle;
		else echo ''; ?>" />
		</a>
	...
	function form($instance)
	{
		$instance = wp_parse_args((array)$instance, array(
			...
			'imageAlt' => 'rtPanel',
			'imageSrc' => 'footer-logo.jpg',
			'linkURL' => '#',
			'imageTitle' => 'rtPanel logo'
		));
...
?>
		...
		echo $title; ?>" /></p>
		<p><label for="<?php
		echo $this->get_field_id('imageTitle'); ?>">Image Title:</label>
		<input class="widefat" id="<?php
		echo $this->get_field_id('imageTitle'); ?>" name="<?php
		echo $this->get_field_name('imageTitle'); ?>" type="text" value="<?php
		echo $imageTitle; ?>" /></p>
		<p><label for="<?php
		echo $this->get_field_id('imageAlt'); ?>">Image Alt:</label>
		<input class="widefat" id="<?php
		echo $this->get_field_id('imageAlt'); ?>" name="<?php
		echo $this->get_field_name('imageAlt'); ?>" type="text" value="<?php
		echo $imageAlt; ?>" /></p>
		<p><label for="<?php
		echo $this->get_field_id('imageSrc'); ?>">Image URL:<br /></label>
		<input class="widefat" id="<?php
		echo $this->get_field_id('imageSrc'); ?>" name="<?php
		echo $this->get_field_name('imageSrc'); ?>" type="text" value="<?php
		echo $imageSrc; ?>" />
		<br /><input class="checkbox" type="checkbox" <?php
		checked($instance['is_template_path'], true) ?> id="<?php
		echo $this->get_field_id('is_template_path'); ?>" name="<?php
		echo $this->get_field_name('is_template_path'); ?>" /><label for="<?php
		echo $this->get_field_id('is_template_path'); ?>">Use Template Path for Image</label></p>
		<p><label for="<?php
		echo $this->get_field_id('linkURL'); ?>">Link URL:<br /></label>
		<?php
	}
}
